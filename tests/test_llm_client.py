...
def test_query_provider_groq_routes_correctly(mock_groq_client):
    with patch("core.llm_client.query_llm", return_value={"raw_response": "ok", "total_tokens": 5}) as mock_q:
...
def test_query_provider_openrouter_routes_correctly():
    with patch("core.llm_client.query_openrouter", return_value="openrouter_response") as mock_or:
...
def test_model_constants_env_override(monkeypatch):
    ...
    # Re-import to pick up new env (or check via importlib.reload)
    import importlib
    import core.llm_client as m
    importlib.reload(m)
