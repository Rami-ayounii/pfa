"""
Final/tests/test_query_parser.py
==================================
Tests for core/query_parser.py (Final/) — NL input -> {domain, location}.
Adapted from Project 2's tests/test_query_parser.py — same import paths.
"""
    ...
    def test_llm_parses_french_query(self):
        ...
        with patch("core.query_parser._get_llm_helpers") as mock_helpers:
            mock_query_llm = MagicMock(
                return_value=_mock_llm_response("Meilleurs cafes", "Tunis"))
            mock_parse_json = MagicMock(
                return_value={"domain": "Meilleurs cafes", "location": "Tunis"})
            mock_helpers.return_value = (mock_query_llm, mock_parse_json, "llama-3.1-8b-instant")

            from core.query_parser import parse_query
            result = parse_query("Meilleurs cafes a Tunis")
