...
_LOCATION_PATTERNS = [
    re.compile(r"^(.+?)\s+(?:in|dans|à|a|en)\s+(.+)$", re.IGNORECASE),
    re.compile(r"^(.+?)\s+(?:near|près de|proche de)\s+(.+)$", re.IGNORECASE),
    re.compile(r"^(.+?)\s+(?:around|autour de)\s+(.+)$", re.IGNORECASE),
    re.compile(r"^(.+?)\s+(?:at|at the)\s+(.+)$", re.IGNORECASE),
]
